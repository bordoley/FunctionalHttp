...
module ClientConnector = 
    let DotNetStreamClient (provideHttpClient:unit -> HttpClient) (request: HttpRequest<Stream>) =
        ...
        async {
            ...
            let! result = provideHttpClient().SendAsync(httpClientRequest, HttpCompletionOption.ResponseHeadersRead) |> Async.AwaitTask |> Async.Catch
