...
module HttpResponseMixins =
    type HttpResponse<'TResp> with
        member this.With<'TResp> (?acceptedRanges:AcceptableRanges,
                                    ...
                                    ?warning: Warning seq) =
            HttpResponse<'TResp>.Create(
                Option.orElse this.AcceptedRanges acceptedRanges,
                Option.orElse this.Age age,
                ...
                Option.orElse this.Date date,
                this.Entity,
                Option.orElse this.ETag etag,
                Option.orElse this.Expires expires,
                defaultArg headers this.Headers,
                defaultArg id this.Id,
                Option.orElse this.LastModified lastModified,
                Option.orElse this.Location location,
                Set.ofSeq <| defaultArg proxyAuthenticate (this.ProxyAuthenticate :> Challenge seq),
                Option.orElse this.RetryAfter retryAfter,
                Option.orElse this.Server server,
                defaultArg status this.Status,
                Option.orElse this.Vary vary,
